...
void compute_G::initialize(const BlockSparsityPattern &in_block_pattern,
  ...
  if (g_matrix.n() != in_matrix.n()) {
    std::cout << "Error in compute_g.cc: length of in_matrix does not match "
                 "length of g_matrix. Check to ensure that in_matrix is reinit "
                 "using in_pattern"
              << std::endl;
    assert(false);
  }
  ...
  if (g_matrix.m() != in_matrix.m()) {
    std::cout
        << "Error in compute_g.cc: width of in_matrix does not match width of "
           "g_matrix. Check to ensure that in_matrix is reinit using in_pattern"
        << std::endl;
    assert(false);
  }
...
void compute_G::get_G_of_U(const BlockVector<double> &U,
                           BlockVector<double> &G_of_U) {
